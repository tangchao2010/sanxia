...
<head>
        ...
        <link href="<%=request.getContextPath()%>/css/wcbstyle5.css" rel="stylesheet" />
        <script>
            (function () {
		...
		<style>
			...
			<!--当前-->
			.wcb_detailtg{ background:#f5f5f5; height:1.2rem; color:#000;}
			.wcb_detailtg dl{ display: -webkit-box;
				display: -webkit-flex;
				display: -ms-flexbox;
				display: flex;
			...
			.wcb_detailtg dl dd{ font-size: .28rem; }
			.wcb_detailtg dl dd:nth-child(1){ width:27%;}
			.wcb_detailtg dl dd:nth-child(2){ width:27%; color:#999;}
			.wcb_detailtg dl dd:nth-child(3){ width:22%;}
			.wcb_detailtg dl dd:nth-child(4){ width:24%; color:#999;font-size:.28rem;}
			.wcb_detailtg:nth-last-child(1){ border-bottom:solid 1px #e3e3e3;}
			.wcb_detailtg{ background: #fff; border-bottom:solid 1px #e3e3e3; }
			...
			.wcb_mytg li:nth-child(1){
				width:27%;	
			}
			.wcb_mytg li:nth-child(2){
				width:26%;	
			}
			.wcb_mytg li:nth-child(3){
				width:22%;	
			}
			.wcb_mytg li:nth-child(4){
				width:25%;	
			}
        </style>
        ...
        //        $('#endTime').date({theme:"datetime"});
            ...
            function Ycallback(seldatatime,isappend){
				...
				$.ajax({
					url:"<%=request.getContextPath()%>/Manage/machInviteInfoJsontwo",
					data:{
...
<input type="hidden" id="beginTime" name="beginTime" value="">
    ...
    <script>
    	$(function(){
			$(".wcb_detailtg  dd").each(function() {
                if( $(this).text() == "入门"){
					$(this).css('color', '#1aabff');	
				}
				if( $(this).text() == "尊享"){
					$(this).css('color', '#ff501a');	
				}
            });
		})
    </script>
 		...
 		//alert(data.payRecordsList.length);
			...
			//alert(document.querySelectorAll("#wrapper2 ul li").length);
				...
				$.each(data.payRecordsList, function(index, item){//遍历数组
					...
					var tmpHtml = '<li class="wcb_detailtg" >'
									+'<dl>'
										+'<dd>'+item.baseOrgName+'</dd>'
										+'<dd>'+item.baseOrgLinkPhone+'</dd>'
										+'<dd>'+item.promotionalGoodsName+'</dd>'
