    ...
    (function ($) {
        // "use strict";
        //TODO::此出开启强制模式会和cropper图像裁剪插件在IOS设备上充突
    ...
    +function ($) {
        ...
        $.initCalendar = function (content) {
            ...
            $content.find("[data-toggle='date']").each(function () {
                ...
                var defData = $(this).attr("data-date");
                var re = /^(\d{4})-(\d{2})-(\d{2})$/g;//正则表达式

                if (re.test(strDate))//判断日期格式符合YYYY-MM-DD
                {
                    var val = {value: [strDate]};
                } else if (re.test(defData)) {
                    var val = {value: [defData]};
                } else {
                    var val = {};
                }
                $(this).calendar(val);
    ...
    +function ($) {
        ...
        //和load 动画方向相反,回退的假像
        Router.prototype.loadBack = function (url, ignoreCache) {
            if (ignoreCache === undefined) {
                ignoreCache = false;
            }
            ...
            if (this._isTheSameDocument(location.href, url)) {
                ...
                this._saveDocumentIntoCache($(document), location.href);
                this._switchToDocument(url, ignoreCache, true, DIRECTION.leftToRight);
            }
        };
        /**
         * 调用 history.forward()
        ...
        Router.prototype._loadDocument = function (url, callback) {
            ...
            this.xhr = $.ajax({
                ...
                success: $.proxy(function (data, status, xhr) {
                    ...
                    var $doc = $('<html></html>');
                    $doc.append(data);
                    //加载新页面时如果有新的token则替换本页的,防止token长时间不更新
                    // var token = $doc.find("meta[name=_token]").attr('content');
                    // if(typeof token == "string"){
                    //     $("html").find("meta[name=_token]").attr('content',token);
                    // }
        ...
        $(function () {
            ...
            $(document).on('click', 'a', function (e) {
                ...
                if ($target.hasClass('back')) {
                    ...
                    var ignoreBack = $target.attr('data-back') === 'true';
                    //动画方向是否回退
                    if(ignoreBack){
                        router.loadBack(url, ignoreCache);
                    }else{
